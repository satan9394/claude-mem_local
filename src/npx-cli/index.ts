...
function parseInstallOptions(argv: string[]): InstallOptions {
  ...
  if (provider !== undefined && provider !== 'claude' && provider !== 'gemini' && provider !== 'openrouter' && provider !== 'cc-switch') {
    console.error(`Unknown --provider: ${provider}. Allowed: claude, gemini, openrouter, cc-switch`);
    process.exit(1);
  }
